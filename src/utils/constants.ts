...
export const API_PATHS = {
  ...
  BOOK_DETAIL: (isbn: string) => `/api/books/detail/${isbn}`,
  BOOK_AVAILABILITY: (isbn: string) => `/api/libraries/book-status?isbn=${isbn}`,
  BESTSELLERS: '/api/books/bestsellers',
  NEW_RELEASES: '/api/books/new-releases',
  USER_LIBRARIES: '/api/libraries/my-library',
  USER_BOOK_STATE: (isbn: string) => `/api/records/book/${isbn}`,
...
/**
 * 한국 시/도 목록 (정보나루 API 형식)
 * 백엔드 RegionCode enum과 일치
 */
...
/**
 * 시/군/구 목록
 * 백엔드 DetailedRegionCode enum과 일치
 * key는 RegionCode의 code 값
 */
export const DISTRICTS: Record<string, readonly { code: string; name: string }[]> = {
  '11': [ // 서울
    { code: '11010', name: '종로구' },
    { code: '11020', name: '중구' },
    { code: '11030', name: '용산구' },
